...
@pytest.fixture
def mock_components():
    with patch("fetchext.inspector.ExtensionInspector") as mock_inspector, \
...
def test_generate_unified_report_success(mock_components, tmp_path):
    # Setup mocks
    mock_components["inspector"].return_value.get_manifest.return_value = {"name": "Test Ext", "version": "1.0"}
    ...
    mock_components["domains"].return_value = {"domains": ["example.com"], "urls": ["http://example.com"]}
...
def test_generate_unified_report_with_yara(mock_components, tmp_path):
    ...
    mock_components["yara"].return_value.scan_archive.return_value = {"file.js": [{"rule": "suspicious"}]}
