...
def test_download_file_new(fs, mock_session):
    ...
    download_file("http://example.com/test.crx", output_path, session=mock_session, show_progress=False)
    
    assert output_path.exists()
    assert output_path.read_bytes() == b"1234567890"
    mock_session.get.assert_called_with("http://example.com/test.crx", stream=True, headers={}, params=None)
