...
def test_migration_golden(case_name, tmp_path):
    ...
    assert actual_manifest == expected_manifest, f"Manifest mismatch for case '{case_name}'"
    ...
    for expected_file in expected_dir.iterdir():
        ...
        assert actual_file.exists(), f"Expected file {expected_file.name} missing in output"
