...
def test_check_update_with_metadata_file(fs, mock_get_downloader, capsys):
    ...
    fs.create_file("extension.crx.json", contents='{"id": "test-id", "version": "1.0.0", "browser": "chrome"}')
    ...
    with patch("fetchext.core.ExtensionInspector") as MockInspector:
        ...
        mock_instance.get_manifest.return_value = {"version": "1.0.0", "browser_specific_settings": {"gecko": {"id": "test-id"}}}
...
def test_check_update_no_update(fs, mock_get_downloader, capsys):
    # Setup
    fs.create_file("extension.crx")
    fs.create_file("extension.crx.json", contents='{"id": "test-id", "version": "2.0.0", "browser": "chrome"}')
...
def test_check_update_infer_from_manifest(fs, mock_get_downloader, capsys):
    ...
    with patch("fetchext.core.ExtensionInspector") as MockInspector:
        ...
        mock_get_downloader.return_value.get_latest_version.assert_called_with("inferred-id")
...
def test_check_update_error_fetching_version(fs, mock_get_downloader, capsys):
    ...
    mock_get_downloader.return_value.get_latest_version.side_effect = Exception("Network error")
