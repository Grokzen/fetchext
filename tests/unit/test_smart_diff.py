...
def test_diff_ignore_whitespace(mock_open, differ):
    ...
    # Mock manifest
    mock_old_zf.read.side_effect = lambda f: b'{"version": "1.0"}' if f == "manifest.json" else b"var x = 1;"
    mock_new_zf.read.side_effect = lambda f: b'{"version": "1.0"}' if f == "manifest.json" else b"var x=1;" # Whitespace change
    ...
    mock_old_zf.read.side_effect = lambda f: b'{"version": "1.0"}' if f == "manifest.json" else b"var x = 1;"
    mock_new_zf.read.side_effect = lambda f: b'{"version": "1.0"}' if f == "manifest.json" else b"var x=1;"
...
def test_diff_images(mock_img_open, mock_open, differ):
    ...
    # Mock manifest
    mock_old_zf.read.side_effect = lambda f: b'{"version": "1.0"}' if f == "manifest.json" else b"image_bytes_old"
    mock_new_zf.read.side_effect = lambda f: b'{"version": "1.0"}' if f == "manifest.json" else b"image_bytes_new"
