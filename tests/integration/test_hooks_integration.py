...
def test_download_extension_cancellation(mock_downloader, mock_hook_manager, tmp_path):
    ...
    result = download_extension("chrome", "http://example.com", tmp_path, show_progress=False)
...
def test_analysis_hooks(mock_hook_manager, tmp_path):
    ...
    with patch("fetchext.inspector.ExtensionInspector"), \
         ...
         patch("fetchext.analysis.domains.analyze_domains", return_value={"domains": [], "urls": []}), \
