...
class TestAnalyzeCLI:
    def test_analyze_complexity(self):
        with (
            patch.object(sys, "argv", ["fext", "analyze", "complexity", "test.crx"]),
    ...
    def test_analyze_entropy(self):
        with (
            patch.object(sys, "argv", ["fext", "analyze", "entropy", "test.crx"]),
            patch("fetchext.analysis.entropy.analyze_entropy") as mock_analyze,
        ):
            mock_analyze.return_value = {"average_entropy": 5.0, "files": []}
    ...
    def test_analyze_domains(self):
        with (
            patch.object(sys, "argv", ["fext", "analyze", "domains", "test.crx"]),
            patch("fetchext.analysis.domains.analyze_domains") as mock_analyze,
        ):
            mock_analyze.return_value = {"domains": [], "urls": []}
    ...
    def test_analyze_yara(self):
        with (
            patch.object(
                sys, "argv", ["fext", "analyze", "yara", "rules.yar", "test.crx"]
            ),
    ...
    def test_analyze_permissions(self):
        with (
            patch.object(sys, "argv", ["fext", "analyze", "permissions", "downloads/"]),
            patch(
                "fetchext.analysis.permissions.PermissionMatrixGenerator"
            ) as MockGenerator,
        ):
