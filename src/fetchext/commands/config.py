def register(subparsers):
    ...
    init_parser.add_argument(
        "-f", "--force", action="store_true", help="Overwrite existing configuration"
    )
    init_parser.set_defaults(func=handle_config_init)

    # Config Remote
    remote_parser = config_subparsers.add_parser(
        "remote", help="Load configuration from a remote URL"
    )
