...
def register(subparsers):
    ...
    sync_parser.add_argument("--url", help="Git repository URL (overrides config)")
    sync_parser.add_argument(
        "--dir", type=Path, help="Local directory to sync to (overrides config)"
    )
...
def handle_sync(args):
    ...
    repo_url = (
        args.url
        or get_config_value(config, "rules.repo_url")
        or "https://github.com/fetchext/community-rules.git"
    )
    ...
    console.print(
        f"Syncing rules from [cyan]{repo_url}[/cyan] to [cyan]{repo_dir}[/cyan]..."
    )
    ...
    try:
        if repo_dir.exists():
            if (repo_dir / ".git").exists():
                ...
                console.print(
                    f"[red]Directory {repo_dir} exists but is not a git repository.[/red]"
                )
