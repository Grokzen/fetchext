...
def register(subparsers):
    ...
    serve_parser = subparsers.add_parser(
        "serve", help="Host local repository as update server"
    )
    ...
    manifest_parser = subparsers.add_parser(
        "update-manifest",
        aliases=["um"],
        help="Generate update manifest for local extensions",
    )
    manifest_parser.add_argument(
        "directory", type=Path, help="Directory containing extension files"
    )
    manifest_parser.add_argument(
        "--base-url", required=True, help="Base URL where extensions are hosted"
    )
    manifest_parser.add_argument(
    ...
    mirror_parser = subparsers.add_parser(
        "mirror", help="Sync local directory with extension list"
    )
    ...
    mirror_parser.add_argument(
        "--prune", action="store_true", help="Remove files not in the list"
    )
    mirror_parser.add_argument(
        "-w",
        "--workers",
