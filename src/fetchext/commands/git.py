...
def register(subparsers):
    ...
    # Init command
    init_parser = git_subparsers.add_parser("init", help="Initialize a git repository for an extension")
    init_parser.add_argument("directory", type=Path, help="Directory to initialize")
    init_parser.add_argument("--no-commit", action="store_true", help="Do not make an initial commit")
...
def handle_init(args):
    ...
    if (target_dir / ".git").exists():
        console.print(f"[yellow]Directory is already a git repository: {target_dir}[/yellow]")
        return

    try:
        # git init
        subprocess.run(["git", "init"], cwd=target_dir, check=True, capture_output=True)
        console.print(f"[green]Initialized empty Git repository in {target_dir}[/green]")
