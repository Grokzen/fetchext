...
def register(subparsers):
    ...
    parser.add_argument(
        "--all", action="store_true", help="Update all tracked extensions"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Check for updates without downloading"
    )
...
def handle_update(args, show_progress=True):
    if not args.all:
        console.print(
            "[red]Currently only --all is supported. Use 'fext check' for single files.[/red]"
        )
    ...
    console.print(
        f"[bold]Checking updates for {len(unique_extensions)} extensions...[/bold]"
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_ext = {
            executor.submit(_check_update, browser, ext_id, entry.get("version")): (
                browser,
                ext_id,
                entry,
            )
                    ...
                    updates_found.append(
                        {
                            ...
                            "source": entry.get(
                                "source"
                            ),  # Note: 'source' might not be in history entry based on history.py
                        }
                    )
    ...
    if args.dry_run:
        for update in updates_found:
            console.print(
                f" - {update['browser']}/{update['id']}: {update['old_version']} -> {update['new_version']}"
            )
        return
    ...
    for update in updates_found:
        console.print(
            f"Downloading update for [cyan]{update['id']}[/cyan] ({update['new_version']})..."
        )
        try:
            ...
            if not target_url or "blob:" in target_url:
                if update["browser"] == "chrome":
                    target_url = (
                        f"https://chromewebstore.google.com/detail/{update['id']}"
                    )
