...
def register(subparsers):
    ...
    audit_parser = subparsers.add_parser(
        "audit", aliases=["a"], help="Audit extension for MV3 compatibility"
    )
    ...
    risk_parser = subparsers.add_parser(
        "risk", aliases=["r"], help="Analyze permission risk"
    )
    ...
    scan_parser = subparsers.add_parser(
        "scan", help="Scan extension for vulnerable dependencies"
    )
    ...
    scan_group.add_argument("--csv", action="store_true", help="Output results as CSV")
    scan_parser.add_argument(
        "--licenses", action="store_true", help="Scan for open source licenses"
    )
    scan_parser.add_argument(
        "--custom", type=Path, help="Path to custom YAML rules file"
    )
    ...
    analyze_subparsers = analyze_parser.add_subparsers(
        dest="analysis_type", required=True, help="Type of analysis"
    )

    # Complexity
    complexity_parser = analyze_subparsers.add_parser(
        "complexity", help="Calculate cyclomatic complexity"
    )
    ...
    # Domains
    domains_parser = analyze_subparsers.add_parser(
        "domains", help="Extract domains and URLs"
    )
    ...
    secrets_parser = analyze_subparsers.add_parser(
        "secrets", help="Scan for potential secrets"
    )
    ...
    yara_parser.add_argument(
        "rules", type=Path, help="Path to YARA rules file or directory"
    )
    ...
    # Permissions
    permissions_parser = analyze_subparsers.add_parser(
        "permissions", help="Generate permission matrix"
    )
    permissions_parser.add_argument(
        "directory", type=Path, help="Directory containing extensions"
    )
    permissions_parser.add_argument(
        "--json", action="store_true", help="Output results as JSON"
    )
    ...
    # Dynamic Analysis
    dynamic_parser = analyze_subparsers.add_parser(
        "dynamic", help="Run dynamic analysis (headless browser)"
    )
    dynamic_parser.add_argument(
        "file", type=Path, help="Path to the extension directory (unpacked)"
    )
    dynamic_parser.add_argument(
        "--headless",
        action="store_true",
    ...
    # API Usage
    api_parser = analyze_subparsers.add_parser(
        "api-usage", help="Analyze Chrome/Browser API usage"
    )
    ...
    report_parser.add_argument(
        "--json", action="store_true", help="Output unified report as JSON"
    )
    report_parser.add_argument(
        "--html", action="store_true", help="Output unified report as HTML"
    )
    report_parser.add_argument(
        "--yara",
        type=Path,
...
def handle_analyze(args, show_progress=True):
    if args.analysis_type == "complexity":
        ...
        if args.json:
            ...
            sorted_files = sorted(
                results["files"], key=lambda x: x["entropy"], reverse=True
            )
        ...
        if args.json:
            ...
            if results:
                ...
                for finding in results:
                    table.add_row(
                        finding.type, finding.file, str(finding.line), finding.match
                    )
                console.print(table)
                console.print(
        ...
        try:
            ...
            console.print(
                f"[green]Dynamic analysis complete. Results saved to {args.output}[/green]"
            )
        except Exception as e:
        ...
        if args.json:
            ...
            if results["imports"]:
                ...
                for imp in results["imports"]:
                    ...
                    table.add_row(
                        imp["module"],
                        imp["field"],
                        kind_map.get(imp["kind"], str(imp["kind"])),
                    )
                console.print(table)
            ...
            if results["exports"]:
                ...
                for exp in results["exports"]:
                    ...
                    table.add_row(
                        exp["name"],
                        kind_map.get(exp["kind"], str(exp["kind"])),
                        str(exp["index"]),
                    )
                console.print(table)
