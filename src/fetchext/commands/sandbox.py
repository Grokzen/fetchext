...
class DenoSandbox:
    ...
    def run(
        ...
        if not self.is_available:
            raise RuntimeError("Deno is not installed. Please install Deno to use the sandbox feature: https://deno.land/")
...
def register(subparsers):
    parser = subparsers.add_parser("sandbox", help="Execute JS in a secure sandbox (requires Deno)")
    ...
    parser.add_argument("--allow-read", action="store_true", help="Allow file system read access")
...
def handle_sandbox(args, show_progress=True):
    ...
    if not sandbox.is_available:
        ...
        console.print("The sandbox feature requires Deno to provide a secure execution environment.")
        console.print("Please install it from [link=https://deno.land/]https://deno.land/[/link]")
        sys.exit(1)
