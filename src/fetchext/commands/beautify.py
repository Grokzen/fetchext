...
def register(subparsers):
    parser = subparsers.add_parser("beautify", aliases=["fmt"], help="Beautify JavaScript and JSON files")
    parser.add_argument("path", help="File or directory to beautify")
    parser.add_argument(
        "-i", "--in-place",
        action="store_true",
        help="Modify file(s) in place"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
        help="Output file path (only for single file)"
    )
...
def handle_beautify(args, show_progress=True):
    ...
    if path.is_dir():
        ...
        if not args.in_place:
            console.print("[yellow]Warning: Beautifying a directory requires --in-place. Assuming --in-place.[/yellow]")
        ...
        result = beautifier.beautify_file(path, in_place=args.in_place, output_path=args.output)
