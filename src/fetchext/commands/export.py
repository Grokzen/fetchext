...
def register(subparsers):
    parser = subparsers.add_parser("export", help="Export analysis data to external formats")
    parser.add_argument("file", type=Path, help="Path to the extension file or directory")
    
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--stix", action="store_true", help="Export as STIX 2.1 Bundle (JSON)")
...
def handle_export(args, show_progress=True):
    if args.stix:
        ...
        console.print(f"[bold blue]Gathering analysis data for {args.file}...[/bold blue]")
