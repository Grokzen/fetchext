...
def register(subparsers):
    ...
    inspect_parser = subparsers.add_parser(
        "inspect", aliases=["i"], help="Inspect an extension file"
    )
    ...
    preview_parser = subparsers.add_parser(
        "preview", aliases=["p"], help="Preview extension contents"
    )
    ...
    diff_parser.add_argument(
        "--visual", action="store_true", help="Generate a visual HTML diff report"
    )
    diff_parser.add_argument(
        "--output",
        type=Path,
...
def handle_inspect(args, show_progress=True):
    core.inspect_extension(
        args.file, show_progress=show_progress, json_output=args.json
    )
    ...
    if args.json:
        ...
        if results["supported_locales"]:
            ...
            for locale in sorted(results["supported_locales"]):
                ...
                status = (
                    "[red]Error[/red]" if "error" in details else "[green]OK[/green]"
                )
