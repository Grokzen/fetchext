...
def analyze_api_usage(file_path: Path, show_progress: bool = False) -> Dict[str, Any]:
    ...
    """
    api_pattern = re.compile(r'\b(chrome|browser)\.([a-zA-Z0-9_]+)(?:\.([a-zA-Z0-9_]+))?')
                ...
                for name in target_files:
                    try:
                        with zf.open(name) as zf_file:
                            ...
                            _scan_content(name, content, api_pattern, api_counts, file_api_map)
                    except Exception:
                        continue
...
def _analyze_directory(directory: Path, pattern: re.Pattern) -> Dict[str, Any]:
    ...
    files = list(directory.rglob("*.js")) + list(directory.rglob("*.html")) + list(directory.rglob("*.htm"))
...
def _scan_content(filename: str, content: str, pattern: re.Pattern, 
                 global_counts: Counter, file_map: Dict):
