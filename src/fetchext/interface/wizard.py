...
def run_setup():
    ...
    save_metadata = Confirm.ask(
        "Save metadata (JSON sidecars) by default?", default=default_metadata
    )
    extract = Confirm.ask(
        "Auto-extract extensions by default?", default=default_extract
    )

    console.print("\n[bold]Batch Settings[/bold]")
    workers = IntPrompt.ask(
        "Default number of workers for batch downloads", default=default_workers
    )

    console.print("\n[bold]Network Settings[/bold]")
    rate_limit_delay = FloatPrompt.ask(
        "Rate limit delay (seconds between requests)", default=default_delay
    )
