...
def generate_update_manifest(
    directory: Path, base_url: str, output_file: Optional[Path] = None
):
    """
        ...
        try:
            ...
            if file_path.suffix == ".crx":
                try:
                    ...
                    logger.warning(
                        f"Could not extract ID from CRX {file_path.name}: {e}"
                    )
            ...
            if not ext_id:
                logger.warning(
                    f"Skipping {file_path.name}: Could not determine Extension ID"
                )
                continue
    ...
    if crx_extensions:
        ...
        out_path = (
            output_file
            if output_file and output_file.suffix == ".xml"
            else directory / "update.xml"
        )
...
def _generate_firefox_json(extensions: List[Dict], base_url: str) -> str:
    ...
    for ext in extensions:
        addons[ext["id"]] = {
            "updates": [
                {"version": ext["version"], "update_link": f"{base_url}/{ext['file']}"}
            ]
        }
