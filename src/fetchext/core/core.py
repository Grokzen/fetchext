...
from fetchext.utils  import open_extension_archive, verify_file_hash, check_disk_space
from fetchext.interface.console  import console, print_manifest_table, print_search_results_table
...
from fetchext.core.verifier  import CrxVerifier, XpiVerifier
...
from fetchext.core.exceptions  import ExtensionError, ConfigError, IntegrityError
...
def download_extension(
    ...
    output_path = downloader.download(
        extension_id, output_dir, show_progress=show_progress
    )
    ...
    if extract:
        extract_extension(
            output_path, output_dir / output_path.stem, show_progress=show_progress
        )
...
def search_extension(
    browser, query, json_output=False, csv_output=False, refresh=False
):
    """
    ...
    if json_output:
        ...
        writer.writerow(
            ["id", "name", "version", "url", "description", "users", "rating"]
        )
...
def check_update(file_path, json_output=False):
    ...
    if metadata_path.exists():
        try:
            with metadata_path.open("r", encoding="utf-8") as f:
                ...
                if not browser:
                    if (
                        "chrome.google.com" in source_url
                        or "chromewebstore.google.com" in source_url
                    ):
                        browser = "chrome"
    ...
    if not extension_id or not browser:
        ...
        if len(file_path.stem) == 32:
            ...
            raise ValueError(
                "Could not determine extension ID or source browser. Please ensure metadata sidecar exists."
            )
    ...
    if json_output:
        ...
        if result["status"] == "update_available":
            console.print(
                f"[{Theme.COLOR_SUCCESS} bold]Update Available![/{Theme.COLOR_SUCCESS} bold] {name} ({local_version} -> {remote_version})"
            )
        elif result["status"] == "up_to_date":
            console.print(
                f"[{Theme.COLOR_INFO} bold]Up to date.[/] {name} ({local_version})"
            )
        elif result["status"] == "check_failed":
            console.print(
                f"[{Theme.COLOR_ERROR} bold]Failed to check for updates.[/] {name}"
            )
        else:
            console.print(
                f"[{Theme.COLOR_WARNING}]Status: {result['status']}[/] {name} ({local_version} vs {remote_version})"
            )
            ...
            if report.issues:
                ...
                for issue in report.issues:
                    color = (
                        ...
                        else Theme.COLOR_WARNING
                        if issue.severity == "warning"
                        else Theme.COLOR_INFO
                    )
                    loc = f"{issue.file}"
                    if issue.line:
                        loc += f":{issue.line}"
                    console.print(
                        f"[{color}]• {issue.message}[/{color}] ([dim]{loc}[/dim])"
                    )
            else:
...
def diff_extensions(
    ...
    try:
        report = differ.diff(
            old_path, new_path, ignore_whitespace=ignore_whitespace, ast_diff=ast_diff
        )
        ...
        if visual:
            ...
            if output_path is None:
                output_path = Path("diff_report.html")
        ...
        if json_output:
            ...
            console.print(
                f"Old Version: {report.old_version} -> New Version: {report.new_version}"
            )
...
def analyze_risk(file_path, json_output=False):
    ...
    try:
        ...
        if json_output:
            ...
            console.print(
                f"Overall Risk Level: [{level_color} bold]{report.max_level}[/{level_color} bold]"
            )
            ...
            if report.risky_permissions:
                ...
                for p in report.risky_permissions:
                    ...
                    console.print(
                        f"  [{p_color}]• {p.permission}[/{p_color}] (Score: {p.score}) - [dim]{p.description}[/dim]"
                    )
...
def verify_signature(file_path, json_output=False):
    ...
    try:
        ...
        result = {"file": str(file_path), "verified": is_valid, "algorithm": algo}
        ...
        if json_output:
            console.print_json(
                data={"file": str(file_path), "verified": False, "error": str(e)}
            )
        else:
...
def batch_download(file_path, output_dir, workers=4, show_progress=True):
    ...
    processor.process(
        file_path, output_dir, max_workers=workers, show_progress=show_progress
    )
    if show_progress:
...
def scan_extension(file_path, json_output=False, csv_output=False):
    ...
    try:
        ...
        if json_output:
            ...
            writer = csv.writer(sys.stdout)
            writer.writerow(
                ["file", "library", "version", "vulnerable", "advisory", "path"]
            )
            ...
            if not report.libraries:
                ...
                for lib in report.libraries:
                    status = (
                        f"[{Theme.COLOR_ERROR}]VULNERABLE[/]"
                        if lib.vulnerable
                        else f"[{Theme.COLOR_SUCCESS}]OK[/]"
                    )
                    console.print(
                        f"  • [bold]{lib.name}[/bold] v{lib.version} ({status})"
                    )
                    console.print(f"    Path: [dim]{lib.path}[/dim]")
                    if lib.vulnerable:
                        console.print(
                            f"    Advisory: [{Theme.COLOR_ERROR}]{lib.advisory}[/]"
                        )
...
def convert_extension(input_path, output_path=None, to_format="zip"):
    ...
    if to_format.lower() != "zip":
        raise ConfigError(
            f"Unsupported target format: {to_format}. Only 'zip' is supported currently."
        )
...
def get_repo_stats(directory, json_output=False):
    ...
    """
    from fetchext.workflow.stats  import RepoAnalyzer, print_stats
    from dataclasses import asdict
...
def generate_html_report(file_path, output_path=None, yara_rules=None):
    ...
    try:
        ...
        raise ExtensionError(
            f"HTML Report generation failed: {e}", original_exception=e
        )
...
def share_report(file_path, provider=None, description=None):
    ...
    """
    from fetchext.sharing  import GistUploader
    ...
    if provider == "gist":
        ...
        if not token:
            raise ConfigError(
                "GitHub token not found. Please set 'sharing.github_token' in config or GITHUB_TOKEN env var."
            )
